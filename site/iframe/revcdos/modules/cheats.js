(function () {
  ...
  function readValue(view, addr, type) {
    try {
      ...
      switch (type) {
        ...
        case "f32":
          return view.getFloat32(addr, true);
        case "i16":
    ...
    if (h.handlerFunc && !h._wrapped) {
      ...
      h.handlerFunc = function (e) {
        if (
          menuOpen &&
          !e._isCheat &&
          (h.eventTypeString.startsWith("key") ||
            h.eventTypeString.startsWith("mouse") ||
            h.eventTypeString.startsWith("touch"))
        ) {
          if (e && e.key === "F3")
    ...
    toggleBtn.addEventListener(
      ...
      (e) => {
        ...
        if (toggleBtnDragging) {
          ...
          newLeft = Math.max(
            0,
            Math.min(window.innerWidth - btnWidth, newLeft),
          );
          newTop = Math.max(
            0,
            Math.min(window.innerHeight - btnHeight, newTop),
          );
  ...
  function firstSearch() {
    ...
    setTimeout(() => {
      ...
      console.log(
        `[CheatEngine] New search: value=${val}, type=${type}, bufferLen=${bufferLen}`,
      );
      ...
      console.log(
        `[CheatEngine] Search complete: found ${results.length} addresses`,
      );
  ...
  function nextSearch() {
    ...
    console.log(
      `[CheatEngine] Next search: value=${val}, checking ${results.length} addresses`,
    );
    ...
    console.log(
      `[CheatEngine] Checked: ${checked}, Skipped: ${skipped}, Found: ${newResults.length}`,
    );
  ...
  function displayResults() {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      try {
        switch (res.type) {
          case "i32":
            currentVal = view.getInt32(res.addr, true);
            break;
          case "f32":
            currentVal = view.getFloat32(res.addr, true).toFixed(2);
            break;
          case "i16":
            currentVal = view.getInt16(res.addr, true);
            break;
          case "i8":
            currentVal = view.getInt8(res.addr);
            break;
          case "f64":
            currentVal = view.getFloat64(res.addr, true).toFixed(2);
            break;
        }
      } catch (e) {
        currentVal = "???";
  ...
  window.editAddr = function (addr, type) {
    const newVal = prompt(
      `Enter new value for 0x${addr.toString(16)} (${type}):`,
    );
    ...
    try {
      switch (type) {
        case "i32":
          view.setInt32(addr, parseInt(newVal), true);
          break;
        case "f32":
          view.setFloat32(addr, parseFloat(newVal), true);
          break;
        case "i16":
          view.setInt16(addr, parseInt(newVal), true);
          break;
        case "i8":
          view.setInt8(addr, parseInt(newVal));
          break;
        case "f64":
          view.setFloat64(addr, parseFloat(newVal), true);
          break;
      }
    } catch (e) {
      alert("Error writing to memory");
    }
    displayResults();
    if (
      document.getElementById("ce-manual-results").style.display === "block"
    ) {
      viewManualAddr();
    }
  };
  ...
  function filterResults(mode) {
    ...
    for (const res of results) {
      ...
      if (mode === "changed" && Math.abs(newVal - oldVal) > 0.0001)
        match = true;
  ...
  function viewManualAddr() {
    ...
    if (isNaN(addr) || addr < 0 || addr >= bufLen - 8) {
      alert(
        "Invalid address (out of range: 0 - 0x" +
          (bufLen - 8).toString(16) +
          ")",
      );
      return;
    }
      ...
      try {
        switch (type) {
          ...
          case "f32":
            ...
            break;
          case "f64":
            val = view.getFloat64(addr, true).toFixed(4);
            break;
        }
  ...
  document.getElementById("ce-changed").onclick = () =>
  ...
  window.typeCheat = async function (code) {
    ...
    const handlers = JSEvents.eventHandlers.filter(
      (h) =>
        h.eventTypeString === "keydown" ||
    ...
    for (let i = 0; i < code.length; i++) {
      ...
      for (const h of handlers) {
        if (h.eventTypeString === "keydown")
          getWasmTableEntry(h.callbackfunc)(
            h.eventTypeId,
            eventDataPtr,
            h.userData,
          );
  ...
  function getStaticAddresses() {
    ...
    const isRu =
      typeof currentLanguage !== "undefined" && currentLanguage === "ru";
  ...
  function setupAirbreak() {
    ...
    if (
      positionAddr < 0 ||
      positionAddr >= bufLen - 12 ||
      headingAddr < 0 ||
      headingAddr >= bufLen - 4
    ) {
      document.getElementById("ce-airbreak-status").textContent =
    ...
    console.log(
      `[AirBreak] Position: X=${x.toFixed(2)}, Y=${y.toFixed(2)}, Z=${z.toFixed(2)}`,
    );
    console.log(
      `[AirBreak] Heading: ${heading.toFixed(2)} rad (${((heading * 180) / Math.PI).toFixed(1)}°)`,
    );
  ...
  function toggleGodMode(e) {
    ...
    document.getElementById("ce-airbreak-status").textContent = godModeEnabled
      ? "GodMode ON - Infinite HP"
      : "GodMode OFF (HP reset to 100)";
    document.getElementById("ce-airbreak-status").style.color = godModeEnabled
      ? "#0f0"
      : "#888";
  }
  ...
  function toggleAirBrake(e) {
    ...
    if (!airbreakShiftAllowed && airbreakEnabled) {
      ...
      document.getElementById("ce-airbreak-status").textContent =
        "AirBreak enabled (RShift to fly)";
      document.getElementById("ce-airbreak-status").style.color = "#0f0";
    } else {
      document.getElementById("ce-airbreak-status").textContent =
        "AirBreak disabled";
  ...
  function toggleFlying() {
    ...
    document.getElementById("ce-airbreak-status").textContent = airbreakEnabled
      ? `FLYING! Z=${lockedZ.toFixed(1)}`
      : "AirBreak enabled (RShift to fly)";
    document.getElementById("ce-airbreak-status").style.color = airbreakEnabled
      ? "#ff0"
      : "#0f0";
  ...
  function airbreakTick() {
    ...
    try {
      ...
      const speed =
        parseFloat(document.getElementById("ce-fly-speed").value) || 2.0;
    ...
    flyToggleBtn.addEventListener(
      ...
      (e) => {
        ...
        if (flyBtnDragging) {
          ...
          newRight = Math.max(
            0,
            Math.min(window.innerWidth - btnWidth, newRight),
          );
          newBottom = Math.max(
            0,
            Math.min(window.innerHeight - btnHeight, newBottom),
          );
        ...
        if (flyBtnDragging) {
          ...
          if (airbreakConfigured) {
            ...
            if (airbreakEnabled) {
              ...
              lockedHealth = view.getFloat32(healthAddr, true);
              console.log(
                `[AirBreak Touch] ENABLED - Locked Z=${lockedZ.toFixed(2)}, HP=${lockedHealth.toFixed(2)}`,
              );
            }
            ...
            document.getElementById("ce-airbreak-status").textContent =
              airbreakEnabled
                ...
                : `Ready! [Tap FLY]`;
            document.getElementById("ce-airbreak-status").style.color =
              airbreakEnabled ? "#ff0" : "#0f0";
