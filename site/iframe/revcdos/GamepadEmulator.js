...
const definePropertiesFromSource = (target, source) =>
  defineProperties(target, getOwnPropertyDescriptors(source));

const definePublicProperty = (obj, key, value) =>
  setProperty(obj, typeof key !== "symbol" ? key + "" : key, value);
...
class GamepadStateTracker {
  constructor(config) {
    ...
    if (
      this.gamepadApiSupported() ||
      navigator.getNativeGamepads !== undefined
    ) {
      this.tickLoop();
    }
  }
  ...
  offGamepadConnect(listener) {
    this.gamepadConnectListeners = this.gamepadConnectListeners.filter(
      (l) => l !== listener,
    );
  ...
  offGamepadDisconnect(listener) {
    this.gamepadDisconnectListeners = this.gamepadDisconnectListeners.filter(
      (l) => l !== listener,
    );
  ...
  offGamepadAxisChange(listener) {
    this.gamepadAxisChangeListeners = this.gamepadAxisChangeListeners.filter(
      (l) => l !== listener,
    );
  }
  ...
  offGamepadButtonChange(listener) {
    this.gamepadButtonChangeListeners =
      this.gamepadButtonChangeListeners.filter((l) => l !== listener);
  }
  ...
  getGamepads() {
    ...
    return getGamepads && typeof getGamepads === "function"
      ? getGamepads.apply(navigator) || []
      : [];
  }
  ...
  cleanup() {
    this.updateDelay = -1;
    this.gamepadConnectListeners.forEach((l) =>
      window.removeEventListener("gamepadconnected", l, true),
    );
    this.gamepadDisconnectListeners.forEach((l) =>
      window.removeEventListener("gamepaddisconnected", l, true),
    );
    this.gamepadConnectListeners = [];
    this.gamepadDisconnectListeners = [];
    this.gamepadButtonChangeListeners = [];
  ...
  checkForAxisChanges(index, gamepad) {
    ...
    for (let i = 0; i < axes.length; i++) {
      ...
      if (currentVal !== prevVal) {
        if (
          Math.abs(currentVal) < this.axisDeadZone &&
          Math.abs(prevVal) < this.axisDeadZone
        ) {
          continue;
        }
        changedAxes[i] = true;
    ...
    if (hasChanges) {
      this.gamepadAxisChangeListeners.forEach((listener) =>
        listener(index, gamepad, changedAxes),
      );
    }
  }
  ...
  checkForButtonChanges(index, gamepad) {
    ...
    if (hasChanges) {
      this.gamepadButtonChangeListeners.forEach((listener) =>
        listener(index, gamepad, changedButtons),
      );
    }
  }
}
...
class GamepadEmulator {
  constructor(buttonPressThreshold) {
    ...
    definePublicProperty(
      this,
      "AddDisplayButtonEventListeners",
      this.AddButtonTouchEventListeners,
    );
    definePublicProperty(
      this,
      "AddDisplayJoystickEventListeners",
      this.AddJoystickTouchEventListeners,
    );
    definePublicProperty(
      this,
      "ClearDisplayButtonEventListeners",
      this.ClearButtonTouchEventListeners,
    );
    definePublicProperty(
      this,
      "ClearDisplayJoystickEventListeners",
      this.ClearJoystickTouchEventListeners,
    );

    this.buttonPressThreshold =
      buttonPressThreshold || this.buttonPressThreshold;
  ...
  gamepadApiNativelySupported() {
    return (
      !!this.getNativeGamepads && !!this.getNativeGamepads.apply(navigator)
    );
  }
  ...
  RemoveEmulatedGamepad(index) {
    ...
    if (gamepad) {
      ...
      const disconnectedGamepad = definePropertiesFromSource(
        mergeObjects({}, gamepad),
        {
          connected: false,
          timestamp: performance.now(),
        },
  ...
  PressButton(gamepadIndex, buttonIndexOrIndices, value, touched) {
    ...
    if (Array.isArray(buttonIndexOrIndices)) {
      const isTouched =
        isPressed ||
        (touched ?? buttons[buttonIndexOrIndices[0]]?.touched) ||
        false;
      for (let i = 0; i < buttonIndexOrIndices.length; i++) {
        const idx = buttonIndexOrIndices[i];
        if (
          idx < 0 ||
          idx >= this.emulatedGamepads[gamepadIndex].buttons.length
        ) {
          console.error(
            "Error: PressButton() - button index " +
              idx +
              " out of range, pass a valid index between 0 and " +
              (this.emulatedGamepads[gamepadIndex].buttons.length - 1),
          );
          continue;
        }
        buttons[idx] = {
      ...
      const isTouched =
        isPressed ||
        (touched ?? buttons[buttonIndexOrIndices]?.touched) ||
        false;
      if (
        buttonIndexOrIndices < 0 ||
        buttonIndexOrIndices >=
          this.emulatedGamepads[gamepadIndex].buttons.length
      ) {
        console.error(
          "Error: PressButton() - button index " +
            buttonIndexOrIndices +
  ...
  AddButtonTouchEventListeners(gamepadIndex, buttonConfigs) {
    ...
    for (let i = 0; i < buttonConfigs.length; i++) {
      ...
      if (!tapTarget) {
        console.warn(
          "GamepadEmulator: No tap target in gamepad " +
            gamepadIndex +
            " display config for button " +
            buttonIndices +
            ", skipping...",
        );
        continue;
      ...
      if (config.type === ControlType.onOff) {
        ...
        const removeDragListener = this.AddDragControlListener(
          config,
          (isDragging, x, y) => {
            let val = isDragging ? this.buttonPressThreshold + 0.00001 : 0;
            val +=
              config.directions[Direction.left] ||
              config.directions[Direction.right]
                ? Math.abs(x)
                : 0;
            val +=
              config.directions[Direction.up] ||
              config.directions[Direction.down]
                ? Math.abs(y)
                : 0;
  ...
  AddDragControlListener(config, callback) {
    ...
    const onPointerMove = (e) => {
      if (activePointerId === e.pointerId) {
        ...
        let { x, y } = normalizeVector(deltaX, deltaY, config.dragDistance);
  ...
  nextEmptyEGpadIndex(overlayMode) {
    ...
    if (overlayMode) {
      ...
      const maxLen = Math.max(
        this.emulatedGamepads.length,
        this.patchedGpadToRealIndexMap.length,
      );
      do {
        if (
          !this.emulatedGamepads[index] &&
          this.patchedGpadToRealIndexMap[index] == null
        )
          break;
        index++;
      } while (index < maxLen);
  ...
  nextEmptyRealGpadIndex(startIndex) {
    ...
    do {
      ...
      const isFree =
        this.realGpadToPatchedIndexMap[index] == null &&
        this.patchedGpadToRealIndexMap[index] == null;
  ...
  monkeyPatchGamepadEvents() {
    ...
    if (window.hasOwnProperty("ongamepadconnected")) {
      originalOnConnectDescriptor = Object.getOwnPropertyDescriptor(
        window,
        "ongamepadconnected",
      );
    ...
    if (window.hasOwnProperty("ongamepaddisconnected")) {
      originalOnDisconnectDescriptor = Object.getOwnPropertyDescriptor(
        window,
    ...
    const handleDisconnect = (event) => {
      ...
      if (gamepad && gamepad.emulation === undefined) {
        ...
        const cloned = this.cloneGamepad(event.gamepad);
        const patchedIndex =
          this.realGpadToPatchedIndexMap[cloned.index] || cloned.index;
    ...
    return function undo() {
      ...
      if (window.hasOwnProperty("ongamepaddisconnected")) {
        Object.defineProperty(
          window,
          "ongamepaddisconnected",
          originalOnDisconnectDescriptor,
        );
    ...
    Object.defineProperty(navigator, "getGamepads", {
      ...
      value: function () {
        ...
        const real = originalGetGamepads
          ? originalGetGamepads.apply(navigator) || []
          : [];
        const result = new Array(Math.max(real.length, emulated.length)).fill(
          null,
        );
        ...
        for (let i = 0; i < real.length; i++) {
          ...
          let patchedIndex =
            self.realGpadToPatchedIndexMap[cloned.index] || cloned.index;
        ...
        for (let i = 0; i < emulated.length; i++) {
          ...
          if (emu && existing) {
            ...
            let btnCount = Math.max(
              existing.buttons?.length || 0,
              emu.buttons.length,
            );
            ...
            for (let j = 0; j < axisCount; j++) {
              ...
              axes[j] =
                Math.abs(emuAxis) > Math.abs(realAxis) ? emuAxis : realAxis;
            }
