...
var ASM_CONSTS = {
  328709: () => {  },
 328710: ($0) => { console.log("BOXEDWINE_FILE:" + UTF8ToString($0)); window.parent.postMessage({ type: "boxedwine-file-read", path: UTF8ToString($0) }, window.location.origin); },
 328873: ($0) => { var str = UTF8ToString($0) + '\n\n' + 'Abort/Retry/Ignore/AlwaysIgnore? [ariA] :'; var reply = window.prompt(str, "i"); if (reply === null) { reply = "i"; } return reply.length === 1 ? reply.charCodeAt(0) : -1; },
 329088: () => { if (typeof(AudioContext) !== 'undefined') { return true; } else if (typeof(webkitAudioContext) !== 'undefined') { return true; } return false; },
 329235: () => { if ((typeof(navigator.mediaDevices) !== 'undefined') && (typeof(navigator.mediaDevices.getUserMedia) !== 'undefined')) { return true; } else if (typeof(navigator.webkitGetUserMedia) !== 'undefined') { return true; } return false; },
 329469: ($0) => { if(typeof(Module['SDL2']) === 'undefined') { Module['SDL2'] = {}; } var SDL2 = Module['SDL2']; if (!$0) { SDL2.audio = {}; } else { SDL2.capture = {}; } if (!SDL2.audioContext) { if (typeof(AudioContext) !== 'undefined') { SDL2.audioContext = new AudioContext(); } else if (typeof(webkitAudioContext) !== 'undefined') { SDL2.audioContext = new webkitAudioContext(); } if (SDL2.audioContext) { if ((typeof navigator.userActivation) === 'undefined') { autoResumeAudioContext(SDL2.audioContext); } } } return SDL2.audioContext === undefined ? -1 : 0; },
 330021: () => { var SDL2 = Module['SDL2']; return SDL2.audioContext.sampleRate; },
 330089: ($0, $1, $2, $3) => { var SDL2 = Module['SDL2']; var have_microphone = function(stream) { if (SDL2.capture.silenceTimer !== undefined) { clearInterval(SDL2.capture.silenceTimer); SDL2.capture.silenceTimer = undefined; SDL2.capture.silenceBuffer = undefined } SDL2.capture.mediaStreamNode = SDL2.audioContext.createMediaStreamSource(stream); SDL2.capture.scriptProcessorNode = SDL2.audioContext.createScriptProcessor($1, $0, 1); SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) { if ((SDL2 === undefined) || (SDL2.capture === undefined)) { return; } audioProcessingEvent.outputBuffer.getChannelData(0).fill(0.0); SDL2.capture.currentCaptureBuffer = audioProcessingEvent.inputBuffer; dynCall('vp', $2, [$3]); }; SDL2.capture.mediaStreamNode.connect(SDL2.capture.scriptProcessorNode); SDL2.capture.scriptProcessorNode.connect(SDL2.audioContext.destination); SDL2.capture.stream = stream; }; var no_microphone = function(error) { }; SDL2.capture.silenceBuffer = SDL2.audioContext.createBuffer($0, $1, SDL2.audioContext.sampleRate); SDL2.capture.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { SDL2.capture.currentCaptureBuffer = SDL2.capture.silenceBuffer; dynCall('vp', $2, [$3]); }; SDL2.capture.silenceTimer = setInterval(silence_callback, ($1 / SDL2.audioContext.sampleRate) * 1000); if ((navigator.mediaDevices !== undefined) && (navigator.mediaDevices.getUserMedia !== undefined)) { navigator.mediaDevices.getUserMedia({ audio: true, video: false }).then(have_microphone).catch(no_microphone); } else if (navigator.webkitGetUserMedia !== undefined) { navigator.webkitGetUserMedia({ audio: true, video: false }, have_microphone, no_microphone); } },
 331782: ($0, $1, $2, $3) => { var SDL2 = Module['SDL2']; SDL2.audio.scriptProcessorNode = SDL2.audioContext['createScriptProcessor']($1, 0, $0); SDL2.audio.scriptProcessorNode['onaudioprocess'] = function (e) { if ((SDL2 === undefined) || (SDL2.audio === undefined)) { return; } if (SDL2.audio.silenceTimer !== undefined) { clearInterval(SDL2.audio.silenceTimer); SDL2.audio.silenceTimer = undefined; SDL2.audio.silenceBuffer = undefined; } SDL2.audio.currentOutputBuffer = e['outputBuffer']; dynCall('vp', $2, [$3]); }; SDL2.audio.scriptProcessorNode['connect'](SDL2.audioContext['destination']); if (SDL2.audioContext.state === 'suspended') { SDL2.audio.silenceBuffer = SDL2.audioContext.createBuffer($0, $1, SDL2.audioContext.sampleRate); SDL2.audio.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { if ((typeof navigator.userActivation) !== 'undefined') { if (navigator.userActivation.hasBeenActive) { SDL2.audioContext.resume(); } } SDL2.audio.currentOutputBuffer = SDL2.audio.silenceBuffer; dynCall('vp', $2, [$3]); SDL2.audio.currentOutputBuffer = undefined; }; SDL2.audio.silenceTimer = setInterval(silence_callback, ($1 / SDL2.audioContext.sampleRate) * 1000); } },
 332957: ($0, $1) => { var SDL2 = Module['SDL2']; var numChannels = SDL2.capture.currentCaptureBuffer.numberOfChannels; for (var c = 0; c < numChannels; ++c) { var channelData = SDL2.capture.currentCaptureBuffer.getChannelData(c); if (channelData.length != $1) { throw 'Web Audio capture buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } if (numChannels == 1) { for (var j = 0; j < $1; ++j) { setValue($0 + (j * 4), channelData[j], 'float'); } } else { for (var j = 0; j < $1; ++j) { setValue($0 + (((j * numChannels) + c) * 4), channelData[j], 'float'); } } } },
 333562: ($0, $1) => { var SDL2 = Module['SDL2']; var buf = $0 >>> 2; var numChannels = SDL2.audio.currentOutputBuffer['numberOfChannels']; for (var c = 0; c < numChannels; ++c) { var channelData = SDL2.audio.currentOutputBuffer['getChannelData'](c); if (channelData.length != $1) { throw 'Web Audio output buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } for (var j = 0; j < $1; ++j) { channelData[j] = HEAPF32[buf + (j*numChannels + c)]; } } },
 334051: ($0) => { var SDL2 = Module['SDL2']; if ($0) { if (SDL2.capture.silenceTimer !== undefined) { clearInterval(SDL2.capture.silenceTimer); } if (SDL2.capture.stream !== undefined) { var tracks = SDL2.capture.stream.getAudioTracks(); for (var i = 0; i < tracks.length; i++) { SDL2.capture.stream.removeTrack(tracks[i]); } } if (SDL2.capture.scriptProcessorNode !== undefined) { SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) {}; SDL2.capture.scriptProcessorNode.disconnect(); } if (SDL2.capture.mediaStreamNode !== undefined) { SDL2.capture.mediaStreamNode.disconnect(); } SDL2.capture = undefined; } else { if (SDL2.audio.scriptProcessorNode != undefined) { SDL2.audio.scriptProcessorNode.disconnect(); } if (SDL2.audio.silenceTimer !== undefined) { clearInterval(SDL2.audio.silenceTimer); } SDL2.audio = undefined; } if ((SDL2.audioContext !== undefined) && (SDL2.audio === undefined) && (SDL2.capture === undefined)) { SDL2.audioContext.close(); SDL2.audioContext = undefined; } },
 335057: ($0, $1, $2) => { var w = $0; var h = $1; var pixels = $2; if (!Module['SDL2']) Module['SDL2'] = {}; var SDL2 = Module['SDL2']; if (SDL2.ctxCanvas !== Module['canvas']) { SDL2.ctx = Browser.createContext(Module['canvas'], false, true); SDL2.ctxCanvas = Module['canvas']; } if (SDL2.w !== w || SDL2.h !== h || SDL2.imageCtx !== SDL2.ctx) { SDL2.image = SDL2.ctx.createImageData(w, h); SDL2.w = w; SDL2.h = h; SDL2.imageCtx = SDL2.ctx; } var data = SDL2.image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = 0xff; src++; dst += 4; } } else { if (SDL2.data32Data !== data) { SDL2.data32 = new Int32Array(data.buffer); SDL2.data8 = new Uint8Array(data.buffer); SDL2.data32Data = data; } var data32 = SDL2.data32; num = data32.length; data32.set(HEAP32.subarray(src, src + num)); var data8 = SDL2.data8; var i = 3; var j = i + 4*num; if (num % 8 == 0) { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; } } else { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; } } } SDL2.ctx.putImageData(SDL2.image, 0, 0); },
 336523: ($0, $1, $2, $3, $4) => { var w = $0; var h = $1; var hot_x = $2; var hot_y = $3; var pixels = $4; var canvas = document.createElement("canvas"); canvas.width = w; canvas.height = h; var ctx = canvas.getContext("2d"); var image = ctx.createImageData(w, h); var data = image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = (val >> 24) & 0xff; src++; dst += 4; } } else { var data32 = new Int32Array(data.buffer); num = data32.length; data32.set(HEAP32.subarray(src, src + num)); } ctx.putImageData(image, 0, 0); var url = hot_x === 0 && hot_y === 0 ? "url(" + canvas.toDataURL() + "), auto" : "url(" + canvas.toDataURL() + ") " + hot_x + " " + hot_y + ", auto"; var urlBuf = _malloc(url.length + 1); stringToUTF8(url, urlBuf, url.length + 1); return urlBuf; },
 337511: ($0) => { if (Module['canvas']) { Module['canvas'].style['cursor'] = UTF8ToString($0); } },
 337594: () => { if (Module['canvas']) { Module['canvas'].style['cursor'] = 'none'; } },
 337663: () => { return window.innerWidth; },
 337693: () => { return window.innerHeight; },
 337724: ($0, $1) => { alert(UTF8ToString($0) + "\n\n" + UTF8ToString($1)); }
};
function boxedwineProcessEvent(type,processId,parentProcessId,launchToken,executable) { window.dispatchEvent(new CustomEvent("boxedwine-native-process", { detail: { type: UTF8ToString(type), processId, parentProcessId, launchToken, executable: UTF8ToString(executable) }, })); }
function boxedwineWindowLifecycle(type,id,parentId,processId,x,y,width,height,dialog) { window.dispatchEvent(new CustomEvent("boxedwine-native-window", { detail: { type: UTF8ToString(type), id, parentId, processId, x, y, width, height, dialog: Boolean(dialog), }, })); }
function boxedwineWindowMetadata(lifecycleType,id,parentId,ownerId,processId,parentProcessId,launchToken,x,y,width,height,frameLeft,frameTop,frameRight,frameBottom,dialog,canResize,canMaximize,canMinimize) { if (!launchToken) { launchToken = Module.boxedwineInitialLaunchToken || Number(new URLSearchParams(window.location.search).get("launchToken")) || 0; } window.dispatchEvent(new CustomEvent("boxedwine-native-window", { detail: { type: "metadata", lifecycleType: UTF8ToString(lifecycleType), id, parentId, ownerId, processId, parentProcessId, launchToken, outerX: x, outerY: y, outerWidth: width + frameLeft + frameRight, outerHeight: height + frameTop + frameBottom, clientX: x + frameLeft, clientY: y + frameTop, clientWidth: width, clientHeight: height, frameLeft, frameTop, frameRight, frameBottom, dialog: Boolean(dialog), canResize: Boolean(canResize), canMaximize: Boolean(canMaximize), canMinimize: Boolean(canMinimize), }, })); }
function boxedwineWindowTitle(id,value,length) { const bytes = HEAPU8.slice(value, value + length); window.dispatchEvent(new CustomEvent("boxedwine-native-window", { detail: { type: "title", id, title: new TextDecoder("windows-1252").decode(bytes).split(String.fromCharCode(0), 1)[0], }, })); }
function boxedwineWindowFrameReady(id,width,height,generation) { window.dispatchEvent(new CustomEvent("boxedwine-native-window", { detail: { type: "frame", id, width, height, generation }, })); }
function boxedwineInstallBridgeApi() { Module.boxedwineInitialLaunchToken = Number(new URLSearchParams(window.location.search).get("launchToken")) || 0; Module.boxedwineSetInitialLaunchToken = (launchToken) => { launchToken = Number(launchToken); if (!Number.isInteger(launchToken) || launchToken <= 0) return 0; Module.boxedwineInitialLaunchToken = launchToken; return 1; }; Module.boxedwineLaunchProcess = (executable, launchToken) => { launchToken = Number(launchToken); if (!Number.isInteger(launchToken) || launchToken <= 0) return 0; const length = lengthBytesUTF8(executable); const pointer = _malloc(length + 1); stringToUTF8(executable, pointer, length + 1); const result = _boxedwine_launch_process(pointer, length, launchToken); _free(pointer); return result; }; Module.boxedwineExpectExec = (filename, launchToken) => { launchToken = Number(launchToken); if (!Number.isInteger(launchToken) || launchToken <= 0) return 0; const length = lengthBytesUTF8(filename); const pointer = _malloc(length + 1); stringToUTF8(filename, pointer, length + 1); const result = _boxedwine_expect_exec(pointer, length, launchToken); _free(pointer); return result; }; Module.boxedwineFrames = Object.freeze({ setProcessVisible: (processId, visible) => _boxedwine_set_process_visible(processId, visible ? 1 : 0), read: (id, previousGeneration = 0) => { const generation = _boxedwine_frame_generation(id); if (!generation || generation === previousGeneration) return null; const pointer = _boxedwine_frame_pointer(id); const width = _boxedwine_frame_width(id); const height = _boxedwine_frame_height(id); if (!pointer || !width || !height) return null; return { generation, width, height, rgba: new Uint8ClampedArray( HEAPU8.buffer, pointer, width * height * 4, ), }; }, }); }

// Imports from the Wasm binary.
var _boxedwine_launch_process = Module['_boxedwine_launch_process'] = makeInvalidEarlyAccess('_boxedwine_launch_process');
var _boxedwine_expect_exec = Module['_boxedwine_expect_exec'] = makeInvalidEarlyAccess('_boxedwine_expect_exec');
var _boxedwine_launch_result = Module['_boxedwine_launch_result'] = makeInvalidEarlyAccess('_boxedwine_launch_result');
var _boxedwine_terminate_process = Module['_boxedwine_terminate_process'] = makeInvalidEarlyAccess('_boxedwine_terminate_process');
var _boxedwine_process_running = Module['_boxedwine_process_running'] = makeInvalidEarlyAccess('_boxedwine_process_running');
var _boxedwine_resize_screen = Module['_boxedwine_resize_screen'] = makeInvalidEarlyAccess('_boxedwine_resize_screen');
var _boxedwine_activate_window = Module['_boxedwine_activate_window'] = makeInvalidEarlyAccess('_boxedwine_activate_window');
var _boxedwine_window_command = Module['_boxedwine_window_command'] = makeInvalidEarlyAccess('_boxedwine_window_command');
var _main = Module['_main'] = makeInvalidEarlyAccess('_main');
var _boxedwine_key_event = Module['_boxedwine_key_event'] = makeInvalidEarlyAccess('_boxedwine_key_event');
var _boxedwine_install_bridge_api = Module['_boxedwine_install_bridge_api'] = makeInvalidEarlyAccess('_boxedwine_install_bridge_api');
var _boxedwine_set_process_visible = Module['_boxedwine_set_process_visible'] = makeInvalidEarlyAccess('_boxedwine_set_process_visible');
var _boxedwine_frame_generation = Module['_boxedwine_frame_generation'] = makeInvalidEarlyAccess('_boxedwine_frame_generation');
var _boxedwine_frame_pointer = Module['_boxedwine_frame_pointer'] = makeInvalidEarlyAccess('_boxedwine_frame_pointer');
var _boxedwine_frame_width = Module['_boxedwine_frame_width'] = makeInvalidEarlyAccess('_boxedwine_frame_width');
var _boxedwine_frame_height = Module['_boxedwine_frame_height'] = makeInvalidEarlyAccess('_boxedwine_frame_height');
...
function assignWasmExports(wasmExports) {
  assert(typeof wasmExports['boxedwine_launch_process'] != 'undefined', 'missing Wasm export: boxedwine_launch_process');
  assert(typeof wasmExports['boxedwine_expect_exec'] != 'undefined', 'missing Wasm export: boxedwine_expect_exec');
  assert(typeof wasmExports['boxedwine_launch_result'] != 'undefined', 'missing Wasm export: boxedwine_launch_result');
  assert(typeof wasmExports['boxedwine_terminate_process'] != 'undefined', 'missing Wasm export: boxedwine_terminate_process');
  assert(typeof wasmExports['boxedwine_process_running'] != 'undefined', 'missing Wasm export: boxedwine_process_running');
  assert(typeof wasmExports['boxedwine_resize_screen'] != 'undefined', 'missing Wasm export: boxedwine_resize_screen');
  assert(typeof wasmExports['boxedwine_activate_window'] != 'undefined', 'missing Wasm export: boxedwine_activate_window');
  assert(typeof wasmExports['boxedwine_window_command'] != 'undefined', 'missing Wasm export: boxedwine_window_command');
  assert(typeof wasmExports['__main_argc_argv'] != 'undefined', 'missing Wasm export: __main_argc_argv');
  assert(typeof wasmExports['boxedwine_key_event'] != 'undefined', 'missing Wasm export: boxedwine_key_event');
  assert(typeof wasmExports['boxedwine_install_bridge_api'] != 'undefined', 'missing Wasm export: boxedwine_install_bridge_api');
  assert(typeof wasmExports['boxedwine_set_process_visible'] != 'undefined', 'missing Wasm export: boxedwine_set_process_visible');
  assert(typeof wasmExports['boxedwine_frame_generation'] != 'undefined', 'missing Wasm export: boxedwine_frame_generation');
  assert(typeof wasmExports['boxedwine_frame_pointer'] != 'undefined', 'missing Wasm export: boxedwine_frame_pointer');
  assert(typeof wasmExports['boxedwine_frame_width'] != 'undefined', 'missing Wasm export: boxedwine_frame_width');
  assert(typeof wasmExports['boxedwine_frame_height'] != 'undefined', 'missing Wasm export: boxedwine_frame_height');
  ...
  _boxedwine_launch_process = Module['_boxedwine_launch_process'] = createExportWrapper('boxedwine_launch_process', wasmExports['boxedwine_launch_process'], 3);
  _boxedwine_expect_exec = Module['_boxedwine_expect_exec'] = createExportWrapper('boxedwine_expect_exec', wasmExports['boxedwine_expect_exec'], 3);
  _boxedwine_launch_result = Module['_boxedwine_launch_result'] = createExportWrapper('boxedwine_launch_result', wasmExports['boxedwine_launch_result'], 0);
  _boxedwine_terminate_process = Module['_boxedwine_terminate_process'] = createExportWrapper('boxedwine_terminate_process', wasmExports['boxedwine_terminate_process'], 1);
  _boxedwine_process_running = Module['_boxedwine_process_running'] = createExportWrapper('boxedwine_process_running', wasmExports['boxedwine_process_running'], 1);
  _boxedwine_resize_screen = Module['_boxedwine_resize_screen'] = createExportWrapper('boxedwine_resize_screen', wasmExports['boxedwine_resize_screen'], 2);
  _boxedwine_activate_window = Module['_boxedwine_activate_window'] = createExportWrapper('boxedwine_activate_window', wasmExports['boxedwine_activate_window'], 1);
  _boxedwine_window_command = Module['_boxedwine_window_command'] = createExportWrapper('boxedwine_window_command', wasmExports['boxedwine_window_command'], 6);
  _main = Module['_main'] = createExportWrapper('__main_argc_argv', wasmExports['__main_argc_argv'], 2);
  _boxedwine_key_event = Module['_boxedwine_key_event'] = createExportWrapper('boxedwine_key_event', wasmExports['boxedwine_key_event'], 3);
  _boxedwine_install_bridge_api = Module['_boxedwine_install_bridge_api'] = createExportWrapper('boxedwine_install_bridge_api', wasmExports['boxedwine_install_bridge_api'], 0);
  _boxedwine_set_process_visible = Module['_boxedwine_set_process_visible'] = createExportWrapper('boxedwine_set_process_visible', wasmExports['boxedwine_set_process_visible'], 2);
  _boxedwine_frame_generation = Module['_boxedwine_frame_generation'] = createExportWrapper('boxedwine_frame_generation', wasmExports['boxedwine_frame_generation'], 1);
  _boxedwine_frame_pointer = Module['_boxedwine_frame_pointer'] = createExportWrapper('boxedwine_frame_pointer', wasmExports['boxedwine_frame_pointer'], 1);
  _boxedwine_frame_width = Module['_boxedwine_frame_width'] = createExportWrapper('boxedwine_frame_width', wasmExports['boxedwine_frame_width'], 1);
  _boxedwine_frame_height = Module['_boxedwine_frame_height'] = createExportWrapper('boxedwine_frame_height', wasmExports['boxedwine_frame_height'], 1);
...
var wasmImports = {
  ...
  /** @export */
  boxedwineProcessEvent,
  /** @export */
  boxedwineWindowFrameReady,
  /** @export */
  boxedwineWindowLifecycle,
  /** @export */
  boxedwineWindowMetadata,
  /** @export */
  boxedwineWindowTitle,
  /** @export */
