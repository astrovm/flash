...
function boxedwineWindowLifecycle(type,id,parentId,processId,x,y,width,height) { window.dispatchEvent(new CustomEvent("boxedwine-native-window", { detail: { type: UTF8ToString(type), id, parentId, processId, x, y, width, height, }, })); }
...
function boxedwineWindowFrame(id,source,pitch,width,height) { const rgba = new Uint8ClampedArray(width * height * 4); for (let y = 0; y < height; y++) { let src = source + y * pitch; let dst = y * width * 4; for (let x = 0; x < width; x++, src += 4, dst += 4) { rgba[dst] = HEAPU8[src + 2]; rgba[dst + 1] = HEAPU8[src + 1]; rgba[dst + 2] = HEAPU8[src]; rgba[dst + 3] = 255; } } window.dispatchEvent(new CustomEvent("boxedwine-native-window", { detail: { type: "frame", id, width, height, rgba }, })); }
...
function assignWasmExports(wasmExports) {
  ...
  _boxedwine_launch_process = Module['_boxedwine_launch_process'] = createExportWrapper('boxedwine_launch_process', wasmExports['boxedwine_launch_process'], 1);
