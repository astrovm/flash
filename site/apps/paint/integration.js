...
function tidySeparators(menu) {
  ...
  rows.forEach((row, index) => {
    ...
    const hasFollowingItem = rows
      .slice(index + 1)
      .some(
        (candidate) => candidate.isConnected && candidate.matches(".menu-item"),
      );
...
function matchXPMenuStructure() {
  ...
  document
    .getElementById(extrasButton?.getAttribute("aria-controls"))
    ?.remove();
  extrasButton?.remove();
  document
    .querySelectorAll(".menu-popup[data-semantic-parent]")
    .forEach((menu) => {
...
function installXPScrollbars() {
  ...
  if (!shell || !viewport || shell.querySelector(".xp-canvas-scrollbar"))
    return;
  ...
  const makeScrollbar = (axis) => {
    ...
    track.addEventListener("pointerdown", (event) => {
      ...
      const offset =
        event[coordinateKey] - track.getBoundingClientRect()[positionKey];
      moveBy(
        offset <
          thumb.getBoundingClientRect()[positionKey] -
            track.getBoundingClientRect()[positionKey]
